...
INFLUXDB_URL = os.getenv("INFLUXDB_V2_URL", "http://influxdb:8086")
INFLUXDB_TOKEN = os.getenv("INFLUXDB_V2_TOKEN", "my-super-secret-auth-token")
INFLUXDB_ORG = os.getenv("INFLUXDB_V2_ORG", "my-org")
INFLUXDB_BUCKET = os.getenv("INFLUXDB_V2_BUCKET", "my-bucket")
RAW_MEASUREMENT = "raw_ticker_data"

masked_token = f"{'*' * (len(INFLUXDB_TOKEN) - 4)}{INFLUXDB_TOKEN[-4:]}" if len(INFLUXDB_TOKEN) > 4 else "****"
logger.info(f"InfluxDB Config Loaded: URL={INFLUXDB_URL}, Org={INFLUXDB_ORG}, Bucket={INFLUXDB_BUCKET}, Token={masked_token}")


# --- Pydantic Models (Keep as before) ---
# Ticker Write Models
class TickerWriteRequest(BaseModel):
    DateTime_UTC: datetime = Field(..., examples=["2024-11-21T16:00:00Z"])
    TICKER_ID: str = Field(..., examples=["SOL-PHP"])
    PRICE: float = Field(..., examples=[115.64])
...
class GetCandlesRequest(BaseModel):
    startDate: date = Field(..., examples=["2024-11-22"])
    endDate: date = Field(..., examples=["2024-11-24"])
...
class CandleResponseItem(BaseModel):
    open: str = Field(..., examples=["115.64"])
    close: str = Field(..., examples=["88.48"])
    high: str = Field(..., examples=["129.45"])
    low: str = Field(..., examples=["79.89"])
    openDateTime_UTC: str = Field(..., examples=["2024-11-22T16:00:00Z"])
    closeDateTime_UTC: str = Field(..., examples=["2024-11-23T15:59:59Z"])

class GetCandlesResponse(BaseModel):
    candles: List[CandleResponseItem]
    startDatetime_UTC_Interpreted: str = Field(..., examples=["2024-11-21T16:00:00Z"])
    endDatetime_UTC_Interpreted: str = Field(..., examples=["2024-11-25T15:59:59Z"])
    TICKER_ID: str
    timezone: str
...
app = FastAPI(
    ...
    version="2.1.1", # Incremented version for query fix
)
...
async def log_requests_responses(request: Request, call_next):
    ...
    try:
        ...
        process_time = system_time.time() - start_process_time
        status_code = 503
        detail = f"InfluxDB Error: Status {e.response.status if e.response else 'N/A'} - {e.message or str(e)}"
...
async def write_ticker_data(request: TickerWriteRequest):
    ...
    try:
        ...
        err_detail = f"Status {e.response.status if e.response else 'N/A'} - {e.message or str(e)}"
        logger.error(f"InfluxDB WRITE Error for {ticker_id}: {err_detail}", exc_info=True)
        raise HTTPException(status_code=503, detail=f"Failed to write data to InfluxDB: {err_detail}")
    except Exception as e:
...
async def get_ticker_data(
    ticker_id: str = Path(..., examples=["SOL-PHP"]),
    start_utc: datetime = Query(..., examples=["2024-11-21T00:00:00Z"]),
    end_utc: datetime = Query(..., examples=["2024-11-23T23:59:59Z"]),
    ...
    try:
        ...
        err_detail = f"Status {e.response.status if e.response else 'N/A'} - {e.message or str(e)}"
        logger.error(f"InfluxDB GET Raw Error for {ticker_id}: {err_detail}", exc_info=True)
        raise HTTPException(status_code=503, detail=f"Failed to query raw data from InfluxDB: {err_detail}")
    except Exception as e:
...
async def get_candles(
    ...
    calc_timezone_str = request.timezone
    ...
    try:
        ...
        end_dt_filter_local = filter_tz.localize(end_dt_naive)
    ...
    # ========================================
    # == MODIFIED FLUX QUERY SECTION START ==
    # ========================================
    flux_query = f"""
        import "timezone"
        ...
        data = from(bucket: "{INFLUXDB_BUCKET}")
            |> range(start: {start_utc_interpreted_str}, stop: {end_utc_interpreted_str})
            |> filter(fn: (r) => r["_measurement"] == "{RAW_MEASUREMENT}")
            |> filter(fn: (r) => r["ticker_id"] == "{ticker_id}")
            |> filter(fn: (r) => r["_field"] == "price")

        // Calculate individual aggregate streams, mapping the _field correctly
        ...
        // Union the streams together into one long table
        union(tables: [open, high, low, close])
            // Pivot the long table into a wide table with OHLC columns
            // Use _time as the row key, _field becomes the column names, _value fills the cells
            |> pivot(rowKey:["_time"], columnKey: ["_field"], valueColumn: "_value")
            // Drop intermediate columns that are constant after pivoting within the same time window
            |> drop(columns: ["_start", "_stop", "_measurement", "ticker_id"])
            // Sort by time for predictable output order
            |> sort(columns: ["_time"])
            |> yield(name: "ohlc_candles")
    """
    # ========================================
    # == MODIFIED FLUX QUERY SECTION END ==
    # ========================================
    ...
    try:
        ...
        if not result:
            ...
            for table in result:
                 ...
                 for record in table.records:
                    # _time from pivot is the start of the window (UTC)
                    open_dt_utc = record.get_time().astimezone(timezone.utc)
                    # Calculate close time based on interval
                    close_dt_utc = open_dt_utc + interval_delta - timedelta(microseconds=1) # NEW
                    ...
                    # Ensure all OHLC values are present before creating response item
                    if o is None or h is None or l is None or c is None:
                         logger.warning(f"Skipping candle at {open_dt_utc} due to missing OHLC values after pivot. Record: {record.values}")
                         continue

                    price_format = ".4f" # Adjust precision as needed
        ...
        err_detail = f"Status {e.response.status if e.response else 'N/A'} - {e.message or str(e)}"
        logger.error(f"InfluxDB GET Candles Error for {ticker_id}: {err_detail}", exc_info=True)
        raise HTTPException(status_code=503, detail=f"Failed to query candle data from InfluxDB: {err_detail}")
    except Exception as e:
    ...
    return GetCandlesResponse(
        ...
        startDatetime_UTC_Interpreted=start_dt_filter_local.astimezone(timezone.utc).strftime('%Y-%m-%dT%H:%M:%SZ'),
...
if __name__ == "__main__":
    ...
    logger.info(f"Default request timezone: {DEFAULT_TIMEZONE}")
