...
def parse_fix_message(msg: str) -> dict:
    ...
    If tag 279 is not present or its value is not "0", return None.
    Otherwise, build and return a command dictionary with the following mapping:
        - command: "PLACE_ORDER"
        - orderType: "GTC"
        - userId: 1
        - userType: "MM"
        ...
        - side: "BID" if tag 269 equals "0", "ASK" if tag 269 equals "1"
        - price: value from tag 270 (as a float)
        - size: value from tag 271 (as a float)
        
    Note: Field 52 is intentionally ignored.
    """
    ...
    # Process message only if tag 279 exists and equals "0"
    ...
    # Determine the order side from tag 269: "0" -> BID, "1" -> ASK
    tag_269 = fix_dict.get("269", None)
    if tag_269 == "0":
        command["side"] = "BID"
    elif tag_269 == "1":
        command["side"] = "ASK"
    else:
        command["side"] = "UNKNOWN"

    # Convert tag 270 (price) and tag 271 (size) to float, if available.
    try:
        command["price"] = float(fix_dict.get("270", "0"))
    except ValueError:
        command["price"] = 0.0

    try:
        command["size"] = float(fix_dict.get("271", "0"))
    except ValueError:
        command["size"] = 0.0
...
def webhook_sender():
    ...
    from the global queue, parses them using parse_fix_message(), and if tag 279 equals "0",
    sends the resulting command dict via an HTTP POST to the specified webhook URL.
    """
    ...
    while True:
        ...
        if parsed_command is not None:
            ...
            print("Webhook sender: Ignored message because tag 279 != 0 or tag 279 is missing.")
        app_in_queue.task_done()
